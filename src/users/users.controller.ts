...
import { Public, VerifyEmail } from '~/auth/auth.decorator';
...
export class UsersController {
  ...
  resendEmail(@Req() request: Request) {
    return this.usersService.resendEmailActivation((request.user as User).uuid);
  }
  ...
  profile(@Req() request: Request) {
    return this.usersService.findById((request.user as User).uuid);
  }
  ...
  updateProfile(@Req() request: Request, @Body() updateUserDto: UpdateUserDto) {
    return this.usersService.update((request.user as User).uuid, updateUserDto);
  }
